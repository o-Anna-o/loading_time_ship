...
func AuthMiddleware(allowedRoles ...string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if authHeader == "" {
			// если пусто — логируем и возвращаем 401
			logrus.Warn("AuthMiddleware: Missing Authorization header")
			c.AbortWithStatusJSON(http.StatusUnauthorized, gin.H{"error": "Missing Authorization header"})
			return
		}

		// --- извлекаем токен и парсим ---
		tokenStr := strings.TrimPrefix(authHeader, "Bearer ")

		// проверяем реальную строку токена
		logrus.Infof("AuthMiddleware: tokenStr(after TrimPrefix)='%v'", tokenStr)

		claims, err := utils.ParseJWT(tokenStr)
		if err != nil {
			logrus.Warnf("AuthMiddleware: ParseJWT failed for token='%v' err=%v", tokenStr, err)
			c.AbortWithStatusJSON(http.StatusUnauthorized, gin.H{"error": "Invalid or expired token"})
			return
		}

		// успешный разбор токена
		...
		if len(allowedRoles) > 0 {
			...
			if !allowed {
				logrus.Warnf("AuthMiddleware: Access denied for role: %v", claims.Role)
