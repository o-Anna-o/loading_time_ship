...
func (h *ShipHandler) AddShipToRequestShipAPI(c *gin.Context) {
	...
	if !exists {
		logrus.Info("ieragdfisdj;aklsjd;")
	...
	err = db.Where("status = ? AND user_id = ?", "черновик", userID).First(&requestShip).Error
	if err != nil {
	...
	// Проверяем наличие корабля в заявке
	var existingShip ds.ShipInRequest
	err = db.Where("request_ship_id = ? AND ship_id = ?", requestShip.RequestShipID, shipID).
		First(&existingShip).Error
	...
	c.JSON(http.StatusOK, gin.H{
		...
		"data": gin.H{
			...
			"ship_id":         shipID,
			"user_id":         userID, // полезно видеть в ответе
		},
	})
}
