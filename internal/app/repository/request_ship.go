...
// UpdateRequestShipContainers - обновляет только количество контейнеров без расчёта времени
func (r *Repository) UpdateRequestShipContainers(id int, c20 int, c40 int) error {
	return r.db.Model(&ds.RequestShip{}).
		Where("request_ship_id = ?", id).
